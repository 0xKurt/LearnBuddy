...
export default function PracticeScreen() {
  ...
  const finalizeMut = useMutation({
    ...
    // Never surface a raw error string to a child (CLAUDE.md §tone).
    onError: () => toast.error(t('error_body')),
  });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: LB.paper }}>
      <SessionTopBar
        ...
        onExit={() =>
          Alert.alert(t('exit_title'), undefined, [
            { text: t('exit_cancel'), style: 'cancel' },
            {
              text: t('exit_end'),
              style: 'destructive',
