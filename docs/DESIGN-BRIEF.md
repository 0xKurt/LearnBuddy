...
- **Solo adult learner.** A 22-year-old studying for university exams. The account holder _is_ the learner. One profile.
...
**One account, one learner profile.** A household with two children who both want to use the app needs two separate accounts. A parent who wants the app for themselves _and_ their child needs two separate accounts. This is a deliberate simplification.
...
5. **Profile fields.** Name, birth year, grade/level, avatar, language. For a _minor_ profile, the account holder explicitly consents to the processing of that minor's data on the spot.
