...
| `PARTNER_MODEL_LOCATION`              | `europe-west4`          | Region for Vertex partner MaaS models (DeepSeek, Llama, Mistral). Partner models often aren't yet available in `europe-west4` — fall back to a region where the model is published. Gemini calls keep using `GOOGLE_VERTEX_LOCATION`.                                                                                                                                                 |
